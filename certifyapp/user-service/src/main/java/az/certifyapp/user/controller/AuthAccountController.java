package az.certifyapp.user.controller;

import az.certifyapp.user.dto.LoginCredentialsRequest;
import az.certifyapp.user.dto.RegisterSyncRequest;
import az.certifyapp.user.dto.UserResponse;
import az.certifyapp.user.service.AuthAccountService;
import jakarta.validation.Valid;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/api/v1/users")
public class AuthAccountController {

    private final AuthAccountService authAccountService;

    public AuthAccountController(AuthAccountService authAccountService) {
        this.authAccountService = authAccountService;
    }

    @PostMapping("/register-sync")
    @ResponseStatus(HttpStatus.CREATED)
    public UserResponse registerSync(@Valid @RequestBody RegisterSyncRequest request) {
        return authAccountService.registerSync(request);
    }

    @PostMapping("/login")
    public UserResponse login(@Valid @RequestBody LoginCredentialsRequest request) {
        return authAccountService.login(request);
    }
}
